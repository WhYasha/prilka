...
if (Test-Path $artifact) {
    ...
    Copy-Item $artifact "dist-desktop/windows/BeHappySetup.exe"
    Copy-Item $artifact "dist-desktop/windows/BeHappySetup-${version}.exe"

    # Generate SHA256
    $hash = (Get-FileHash "dist-desktop/windows/BeHappySetup.exe" -Algorithm SHA256).Hash
    "$hash  BeHappySetup.exe" | Out-File -Encoding ascii "dist-desktop/windows/BeHappySetup.exe.sha256"

    # Generate latest.json
    @{
        version = $version
        date    = (Get-Date -Format "yyyy-MM-dd")
        url     = "https://behappy.rest/downloads/windows/BeHappySetup.exe"
        sha256  = $hash
    } | ConvertTo-Json | Out-File -Encoding utf8 "dist-desktop/windows/latest.json"

    Write-Host "==> Build complete!"
    Write-Host "    Installer: dist-desktop/windows/BeHappySetup.exe"
