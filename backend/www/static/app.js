/* Simple Messenger v2 – C++ backend SPA client */
"use strict";
...
// ── API base ────────────────────────────────────────────────────────────────
// data-cpp-api is set to "/api" in index.html so requests go through nginx.
const CPP_API = document.body.dataset.cppApi || "/api";

async function cppApiFetch(path, opts = {}) {
  const token = localStorage.getItem("access_token") || localStorage.getItem("cpp_token");
...
async function bootstrap() {
  ...
  // Load stickers
  try {
    const res = await cppApiFetch("/stickers");
  ...
  // Handle deep link from URL path (JS-driven, no Flask needed)
  handlePathDeepLink();
...
// ── Path-based deep link routing (replaces Flask template deeplink) ──────────
function handlePathDeepLink() {
  const path = window.location.pathname;
  if (path.startsWith("/@")) {
    const username = path.slice(2);
    if (username) showUserProfile(username);
  } else if (path.startsWith("/u/")) {
    const userId = path.slice(3);
    if (userId) showUserProfileById(userId);
  } else if (path.startsWith("/dm/")) {
    const target = path.slice(4);
    if (target) openDMDeepLink(target);
  } else if (path.startsWith("/join/")) {
    const token = path.slice(6);
    if (token) showJoinPreview(token);
  } else if (path.startsWith("/c/")) {
    const name = path.slice(3);
    if (name) showChannelByName(name);
  }
}
...
function openChat(chatId) {
  ...
  setAvatar(chatHeaderAvatar, displayName, chat.other_avatar_url || null);
...
async function loadMessages(forceScroll = false) {
  ...
  try {
    // Use after_id for polling, no param for initial load
    const param = S.lastMsgId > 0 ? `?after_id=${S.lastMsgId}&limit=50` : `?limit=50`;
    const res = await cppApiFetch(`/chats/${S.activeChatId}/messages${param}`);
...
function buildBubble(msg) {
  // C++ API uses sender_id; legacy used from_user_id
  const senderId = msg.sender_id !== undefined ? msg.sender_id : msg.from_user_id;
  const isMine = senderId === S.me.id;
  ...
  if (msg.message_type === "sticker") {
    // Look up sticker URL from local cache if not in msg
    let stickerSrc = msg.sticker_url || "";
    let stickerLabel = msg.sticker_label || "sticker";
    if (!stickerSrc && msg.sticker_id) {
      const local = S.stickers.find(s => s.id === msg.sticker_id);
      if (local) { stickerSrc = local.url; stickerLabel = local.label; }
    }
    bubbleContent = `<img class="sticker-img" src="${escHtml(stickerSrc)}" alt="${escHtml(stickerLabel)}" />`;
    row.innerHTML = `
      <div class="msg-bubble sticker-bubble">${bubbleContent}</div>
      <div class="msg-time">${escHtml(formatTime(msg.created_at))}</div>
    `;
  } else if (msg.message_type === "voice") {
    // C++ API provides attachment_url (presigned); legacy used /uploads/ path
    const src = msg.attachment_url || (msg.attachment_path ? `/uploads/${escHtml(msg.attachment_path)}` : "");
...
async function sendText() {
  ...
  try {
    const res = await cppApiFetch(`/chats/${S.activeChatId}/messages`, {
      method: "POST",
    ...
    // Enrich with S.me info for immediate rendering
    msg.sender_id = S.me.id;
    msg.sender_username = S.me.username;
    msg.sender_display_name = S.me.display_name;
    msg.sender_avatar_url = S.me.avatar_url;
...
async function sendSticker(stickerId) {
  ...
  try {
    const res = await cppApiFetch(`/chats/${S.activeChatId}/messages`, {
      method: "POST",
      body: JSON.stringify({ content: "", type: "sticker", sticker_id: stickerId }),
    });
    if (!res.ok) { showToast("Failed to send sticker"); return; }
    const msg = await res.json();
    // Enrich with sticker data from local cache
    msg.sender_id = S.me.id;
    const localSticker = S.stickers.find(s => s.id === stickerId);
    if (localSticker) {
...
async function uploadVoice(file, duration) {
  ...
  try {
    // Step 1: upload file to C++ files endpoint
    const fileRes = await cppApiFetch("/files", { method: "POST", body: fd });
    if (!fileRes.ok) { showToast("Failed to upload voice message"); return; }
    const fileData = await fileRes.json();

    // Step 2: send message with file_id and duration
    const msgRes = await cppApiFetch(`/chats/${S.activeChatId}/messages`, {
      method: "POST",
      body: JSON.stringify({
        content: "",
        type: "voice",
    ...
    if (!msgRes.ok) { showToast("Failed to send voice message"); return; }

    // Reload to get the presigned attachment URL
...
async function openGroupChatForm() {
  ...
  try {
    ...
    groupMemberSearch.oninput = async () => {
      const q = encodeURIComponent(groupMemberSearch.value);
      try {
        const r = await cppApiFetch(`/users/search?q=${q}`);
        _groupUsers = await r.json();
        renderGroupMemberList(_groupUsers);
      } catch {}
    };
...
function renderUserPickList(users) {
  ...
  userList.innerHTML = "";
  users.forEach(u => {
    if (S.me && u.id === S.me.id) return; // skip self
    const div = document.createElement("div");
...
avatarFileInput.addEventListener("change", async () => {
  ...
  try {
    // Step 1: upload file
    const fileRes = await cppApiFetch("/files", { method: "POST", body: fd });
    if (!fileRes.ok) { const d = await fileRes.json(); showToast(d.error || "Upload failed"); return; }
    const fileData = await fileRes.json();

    // Step 2: set as avatar
    const avatarRes = await cppApiFetch("/users/me/avatar", {
      method: "PUT",
      body: JSON.stringify({ file_id: fileData.id }),
    });
    if (!avatarRes.ok) { showToast("Failed to set avatar"); return; }
    const avatarData = await avatarRes.json();
    S.me.avatar_url = avatarData.avatar_url;
    setAvatar(profileAvatar, profileDisplayName.value || S.me.username, avatarData.avatar_url);
    updateDrawerUser();
...
function handleDeepLink(deeplink) {
  ...
  switch (type) {
    case "profile":    showUserProfile(value);   break;
    case "profile-id": showUserProfileById(value); break;
    case "dm":         openDMDeepLink(value);    break;
    case "join":       showJoinPreview(value);   break;
    case "channel":    showChannelByName(value); break;
  }
}
...
async function showUserProfileById(userId) {
  if (!userId) return;
  try {
    const res = await cppApiFetch(`/users/${userId}`);
    if (!res.ok) { showToast("User not found"); return; }
    const user = await res.json();
