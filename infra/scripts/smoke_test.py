...
if token_b:
    req("PUT", "/settings", {"last_seen_visibility": "everyone"}, token=token_b)
...
if token_b and user_b_id and presence_chat:
    ws_presence_code = "\n".join([
        ...
        "        print('ONLINE:' + ('OK' if online else 'FAIL'))",
        "        await drain(ws_a)",
        "        await ws_b.send(json.dumps({'type':'presence_update','status':'away'}))",
        "        offline = await wait_presence(ws_a, USER_B, 'offline')",
        "        if not offline:",
        "            offline = await wait_presence(ws_a, USER_B, 'recently')",
        "        print('OFFLINE:' + ('OK' if offline else 'FAIL'))",
        "        await ws_b.close()",
        "        await ws_a.close()",
        "    except Exception as e:",
        "        print('ERR:' + str(e))",
